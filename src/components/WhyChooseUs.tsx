...
export function WhyChooseUs() {
  return (
    <section className="relative bg-black text-white border-t border-white/5 px-5 sm:px-8 md:px-12 pt-16 sm:pt-24 pb-16 sm:pb-24">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-12 sm:mb-16">
        {PILLARS.map(({ Icon, label, stat, body }) => (
          <div
            key={label}
            className="flex flex-col gap-5 p-6 sm:p-10 rounded-2xl border border-white/5 bg-white/[0.02]"
          >
            ...
            <h3 className="font-outfit font-medium text-xl sm:text-2xl text-white">{label}</h3>
            <p className="text-white/60 text-sm sm:text-base leading-relaxed">{body}</p>
          </div>
        ))}
      </div>

      <div className="border-l-2 border-primary pl-6 sm:pl-10 max-w-4xl">
        <p className="font-outfit text-base sm:text-lg md:text-xl leading-relaxed text-white/80">
          Our clients are based across{" "}
          <span className="text-white font-medium">Hull, Yorkshire, and the rest of the UK</span> — plumbers,
        ...
        <Link
          ...
          className="mt-8 inline-flex items-center gap-2 px-7 py-3 rounded-full bg-primary text-white font-outfit font-medium text-base sm:text-lg hover:bg-white hover:text-primary transition-all duration-300"
        >
